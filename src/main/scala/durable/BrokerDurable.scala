...
object BrokerDurable {
    ...
    def apply(blockchainActor: ActorRef[BlockChainDurable.AddBlockEvent]): Behavior[BrokerEvent] =
        brokerBehavior(blockchainActor, List.empty)

    private def brokerBehavior(
                                  blockchainActor: ActorRef[BlockChainDurable.AddBlockEvent],
