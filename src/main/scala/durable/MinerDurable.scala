...
import BlockChainDurable.{AddBlockEvent, GetLastHashEvent, getIndexEvent}
import BrokerDurable.{ClearTransactionEvent, GetTransactionEvent}
...
object MinerDurable {
    ...
    def apply(brokerActor: ActorRef[BrokerDurable.BrokerEvent], blockchainActor: ActorRef[BlockChainDurable.BlockChainEvent]): Behavior[MinerEvent] =
        minerBehavior(brokerActor, blockchainActor)

    private def minerBehavior(
                                 brokerActor: ActorRef[BrokerDurable.BrokerEvent],
                                 blockchainActor: ActorRef[BlockChainDurable.BlockChainEvent]
                             ): Behavior[MinerEvent] = {
            ...
            Behaviors.receive { (context, message) =>
                message match {
                    case ValidateBlock(block, replyTo) =>
                        val isValid = BlockDurable.isValidProof(block)
                        replyTo ! isValid
                        Behaviors.same
